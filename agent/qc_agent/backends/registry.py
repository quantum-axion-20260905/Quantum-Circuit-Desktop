...
def method_catalog(*, gpu_available: bool, tensor_network_available: bool) -> list[MethodCapability]:
    ...
    return [
        ...
        MethodCapability(
            ...
            limitations=("bounded unit cells up to 2x2; larger cells require a new admission contract", "coordinate, finite-difference-gradient, and SPSA paths are strict infinite-objective experimental baselines; autodiff-ctmrg-gradient is a bounded unrolled Torch autograd baseline; implicit-ctmrg-gradient is a bounded adjoint research path requiring transfer-gap and gauge evidence; finite-torus-gradient is an exact four-site reference/initializer, not an infinite variational solver", "no fallback to finite boundary-MPS"),
        ),
    ]
