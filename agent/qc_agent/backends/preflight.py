...
def estimate_ctmrg(payload: Any, *, gpu_free_mb: float | None = None) -> dict[str, Any]:
    ...
    if bool(getattr(payload, "boundary_mps_transfer_fixed_point", False)):
        boundary_transfer_fixed_point_cycles = int(getattr(payload, "boundary_mps_transfer_cycles", 8))
        transfer_width = int(getattr(payload, "boundary_mps_width", 4))
        transfer_bond_dim = int(getattr(payload, "boundary_mps_bond_dim", 16))
        boundary_transfer_fixed_point_work = (
            boundary_transfer_fixed_point_cycles
            * max(1, cell_sites // max(1, int(payload.unit_cell[0])))
            * max(1, transfer_width)
            * max(1, transfer_bond_dim) ** 3
    ...
    if getattr(payload, "optimization", "none") == "full-update":
        ...
        if optimizer == "spsa-gradient":
            ...
            evaluations_per_parameter = 0
            estimated_full_update_evaluations = 1 + int(getattr(payload, "optimization_steps", 1)) * 5
        elif optimizer == "implicit-ctmrg-gradient":
            evaluations_per_parameter = 0
            estimated_full_update_evaluations = 1 + int(getattr(payload, "optimization_steps", 1)) * 5
        else:
    ...
    elif projector == "covariant-bilinear":
        warnings.append("covariant-bilinear CTMRG projector is an opt-in 1x1 research candidate; reduced-overlap rank, convergence, and paired-gauge gates remain mandatory")
