...
class CTMRGBoundaryMPSTransferStudyPayload(BaseModel):
    """Bounded row-transfer boundary-MPS fixed-point convergence contract."""

    problem: CTMRGPayload
    widths: list[int] = Field(default_factory=lambda: [2], min_length=1, max_length=4)
    boundary_bond_dims: list[int] = Field(default_factory=lambda: [2], min_length=1, max_length=4)
    ...
    def validate_widths(cls, value: list[int]) -> list[int]:
        ...
        if any(item < 1 or item > 8 for item in normalized):
            raise ValueError("boundary-MPS transfer widths must be between 1 and 8")
        if len(set(normalized)) != len(normalized):
    ...
    @field_validator("boundary_bond_dims")
    @classmethod
    def validate_transfer_bond_dims(cls, value: list[int]) -> list[int]:
        normalized = [int(item) for item in value]
        if any(item < 1 or item > 128 for item in normalized):
            raise ValueError("boundary-MPS transfer bond dimensions must be between 1 and 128")
        if len(set(normalized)) != len(normalized):
    ...
    def validate_problem(self):
        if len(self.widths) * len(self.boundary_bond_dims) > 8:
            raise ValueError("boundary-MPS transfer convergence studies are limited to eight points")
        if self.problem.optimization != "none":
            raise ValueError("boundary-MPS transfer convergence studies require problem.optimization='none'")
        if self.problem.environment_sector_policy != "single":
            raise ValueError("boundary-MPS transfer convergence studies and symmetry-sector ensembles are separate experimental policies")
        if self.problem.gauge_preconditioner != "none":
            raise ValueError("boundary-MPS transfer convergence studies and virtual-gauge preconditioning are separate experimental policies")
        return self
